...
The Kohlrausch function is
\[
	\sigma(t) = \sigma_0 \exp\!\left[-\left(\frac{t}{\theta_K}\right)^n\right].
\]
Therefore the relaxation function is
$\varphi(t)=\exp\!\left[-\left(\frac{t}{\theta_K}\right)^n\right]$.
It can also be expressed in terms of is the relaxation-time spectrum $G(\theta)$.
\begin{equation}
	\varphi(t)=\int_0^\infty G(\theta)\,e^{-t/\theta}\,d\theta.
	\label{eq:phi_t}
\end{equation}
This gives us the weighting for each relaxation time which is what we need to solve this problem.
To begin, we will do a change of variables.
Let $s=1/\theta$, so $d\theta=-ds/s^2$, then
\[
...
\]
so that there are $N$ discrete relaxation times $\theta_i$ with weights $G_i$.
...
\[
	...
	\implies
	\sigma(t) = 
	\sigma_0 \sum_{i=1}^N G_i e^{-t/\theta_i}
\]
...
Now we can verify this result by substituting $G(\theta)$ back into equation~\ref{eq:phi_t} and confirming that the integral recovers the Kohlrausch function. For case $n=1/2$ (this is shown in figure~\ref{fig:kwww}) the numerical integration of equation~\ref{eq:pollard_series} is able to reproduce $\varphi(t) = e^{-\sqrt{t/\theta_K}}$.
For higher values of $n$ I was not able to reproduce this due to numerical instabilities form the Gamma function.
\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\textwidth]{figures/kww_half.png}
	\caption{
		The relaxation function $\varphi(t) = e^{-\sqrt{t/\theta_K}}$ (solid line) replicated by the numerical integration of equation~\ref{eq:pollard_series}.
	}
	\label{fig:kwww}
\end{figure}
...
I decided to combine these because the first part of 3-6 entirely covers 3-5.
...
\]
\[
	R^2 \int_0^R r dr = \frac{R^4}{2}, \quad
...
\[
	...
	\implies \sigma_\text{max} =  \frac{4\eta Q}{\pi R^3}
\]
\[
	\dot\gamma_\text{max} = \sigma_\text{max}/\eta = \boxed{\frac{4Q}{\pi R^3}}
\]
...
\Bigg]
\end{align*}
Substituting $\sigma_Y = \frac{\Delta P}{2L}r_0$:
\begin{align*}
Q
&= 2\pi \frac{\Delta P}{\mu L}\Bigg[
  \frac{1}{ 8}(R - r_0)^2 r_0^2
+ \frac{1}{16}R^4 
- \frac{1}{12}R^3r_0
- \frac{1}{ 4}\left(R^2\frac{r_0^2}{2} - \frac{r_0^4}{4}\right)
+ \frac{1}{ 2}r_0\left(R\frac{r_0^2}{2} - \frac{r_0^3}{3}\right)
\Bigg]
\\
&= 2\pi \frac{\Delta P}{\mu L}
\Bigg[
  \frac{1}{ 8}R^2r_0^2 
- \frac{1}{ 4}Rr_0^3
+ \frac{1}{ 8}r_0^4
...
&= 2\pi \frac{\Delta P}{\mu L}
\Bigg[
\cancel{\frac{1}{8}R^2r_0^2} 
- \cancel{\frac{1}{8}R^2r_0^2}
+ \cancel{\frac{1}{4}Rr_0^3}
- \cancel{\frac{1}{4}Rr_0^3}
+ \frac{1}{8}r_0^4
+ \frac{1}{16}R^4 
- \frac{1}{12}R^3r_0 
...
\Bigg]
\end{align*}
gives our final equation for $Q$ in terms of the pressure gradient, the viscosity, the length and the critical radius ($r_0$).
...
The top plate is a damped harmonic oscillator being driven at an angular velocity of $\omega$. The elastic and viscous components of the spring are captured by $Z^* = Z' + iZ''$ and the elastic and viscous components of the material between the plates are captured by $G^* = G' + iG''$.
Its equation of motion is
\begin{equation}
	m\ddot{x}_2 + Z^* x_2 + \frac{S}{h}G^*(x_2 - x_1) = 0
	\label{eq:ode_5-6}
\end{equation}
where $m$ is the mass of the top plate, 
$S$ is the surface area of the plate, 
$h$ is the height of the sample, 
$x_2$ is the displacement of the top plate
and $x_1$ is the displacement of the driving plate.
Since the system is linear the solution for $x_1$ and $x_2$ should have the same angular velocity $\omega$:
\[
	\begin{bmatrix}
		x_1 \\ x_2
	\end{bmatrix}
...
and $\ddot{x}_2 = -c_2\omega^2 e^{i\varphi} e^{i\omega t}$ into equation \ref{eq:ode_5-6}:
\[
...
\[
	G^* = \frac{h}{S} \cdot 
	\frac{[m\omega^2 - Z' - iZ''][(1 - r\cos\varphi) - ir\sin\varphi]}{[(1 - r\cos\varphi) + ir\sin\varphi][(1 - r\cos\varphi) - ir\sin\varphi]}
\]
Just the numerator:
\begin{align*}
	&\quad [m\omega^2 - Z' - iZ''][(1 - r\cos\varphi) - ir\sin\varphi]\\
	&= [m\omega^2 - Z' - iZ''](1 - r\cos\varphi) - [m\omega^2 - Z' - iZ'']ir\sin\varphi\\
	&= (m\omega^2 - Z')(1 - r\cos\varphi) - iZ''(1 - r\cos\varphi) - [i(m\omega^2 - Z')r\sin\varphi + Z''r\sin\varphi]\\
	&= (m\omega^2 - Z')(1 - r\cos\varphi) - iZ''(1 - r\cos\varphi) - i(m\omega^2 - Z')r\sin\varphi - Z''r\sin\varphi \\
	&= (m\omega^2 - Z')(1 - r\cos\varphi) - Z''r\sin\varphi - i[(m\omega^2 - Z')r\sin\varphi + Z''(1 - r\cos\varphi)] \\
\end{align*}
Substituting the numerator back into $G^*$ gives our final results:
\[
	G^* = \frac{h}{S} \cdot 
	\frac{(m\omega^2 - Z')(1 - r\cos\varphi) - Z''r\sin\varphi 
	  - i[(m\omega^2 - Z')r\sin\varphi + Z''(1 - r\cos\varphi)]}{
...
\[
	G'' = - \frac{h}{S} \cdot 
	\frac{(m\omega^2 - Z')r\sin\varphi + Z''(1 - r\cos\varphi)}{
		(1 - r\cos\varphi)^2 + r^2\sin^2\varphi}
\]
\[
	\tan\delta = \frac{G''}{G'} = 
...
According to Malkin, the results are:
\[
	G^* = \frac{(Z^* - m\omega^2)Be^{i\varphi}}{K(A-Be^{i\varphi})},
\]
\[
	G' = \frac{(Z''- m\omega^2)(p\cos\varphi-1)-Z'' p\sin\varphi}{K[(p-\cos\varphi)^2 +\sin^2\varphi]},
\]
\[
	G'' = \frac{(Z' - m\omega^2)p\sin\varphi + Z'(p\cos\varphi - 1)}{K[(p-\cos\varphi)^2 +\sin^2\varphi]}.
\]
These do not agree with my results.
Putting aside differences in notation ($K = S/h$ and $A, B = c_1, c_2$), Malkin says 
that $G'$ is a function of $Z''$ and not $Z'$ and vice versa. Based on my 
calculations, I think this has to be a typo because if you swap one $Z'$ with $Z''$ 
and another $Z''$ with $Z'$ then our results agree. The denominators also look different 
but are really the same\footnote{
Malkin's denominator: $(p - \cos\varphi)^2 + \sin^2\varphi = p^2 - 2p\cos\varphi + \cos^2\varphi + \sin^2\varphi = 1 + p^2 - 2p\cos\varphi$.
Ours: $(1 - r\cos\varphi)^2 + r^2\sin^2\varphi = 1 - 2r\cos\varphi + r^2\cos^2\varphi + r^2\sin^2\varphi = 1 - 2r\cos\varphi + r^2$. These are equal since $r = p$.
}.
